...
| Account claim and walk kernel | Built and verified | `0005_account_and_walk.sql`. `claim_account` is asserted both ways, first claimant admin and every later one cellar, and W-8 walked the screens above it in a browser. The Postgres 16 in the old note was stale twice over: `config.toml` pins 17 and green runs there |

# G-3: offline behaviour, PWA, and a phone in a barrel room
...
`apps/web/index.html`, `apps/web/vite.config.ts`, and `packages/cellar/src/scan.ts`, `walk.ts`, `ui.ts`, plus `packages/core/src/kernel.ts` and `env.ts`. A complete listing of `apps/web/` matters as much as its contents here, since several findings turn on files that do not exist.
...
Build the client and serve it. Check the Host header behaviour against localhost, a LAN IP, and an invented hostname, since that is a three-command test with a definite answer.
...
`apps/web/` in full including `index.html`, `vite.config.ts`, and `src/app.css`;
`packages/cellar/src/scan.ts`, `sticky.ts`, `ui.ts`, and `walk.ts`;
`packages/core/src/kernel.ts` and `env.ts`.

## Context for this review

The client is described as a PWA and is used on phones in a barrel room: cold, dark, wet
hands, gloves, bad signal, and stretches with no connectivity at all. The device may be an
iPad on iPadOS Safari or an Android phone. Sessions can run for hours between sign-ins.

Review it as a field application, not as a website.

## Review this

1. **Is it actually a PWA?** Check for a web app manifest, icons, a service worker,
   registration code, and a caching strategy. State plainly what exists and what does not, and
   what the consequences are for installing it to a home screen, launching it without signal,
   and surviving a reload in a dead zone.

2. **Offline reads and writes.** Determine what the client actually does when a request fails
   because there is no network. Trace it in the code rather than assuming. For each screen:
   does anything queue, does anything retry, does anything persist across a reload, and what
   does the user see. If the answer is nothing, say so, and say what the minimum honest
   version looks like given the deadline.

3. **Session and token lifetime.** Supabase issues a JWT with an expiry and refreshes it. Work
   out what happens to a session that sits idle offline for six hours and then tries to write,
   and what happens when a refresh fails because the device is offline. State whether the user
   is bounced to sign-in and whether unsaved input survives that.

4. **Retry and duplication.** When a request times out and the user presses the button again,
   what is sent, and can the same action land twice? Ids are generated on the client
   specifically so this is answerable. Trace whether that property is actually used at each
   write path.

5. **The camera.** `scan.ts` uses zxing. Review permission handling, the failure path when
   permission is denied, the failure path when there is no camera, behaviour on a wet or dirty
   lens, focus and lighting in a dark room, torch access, and whether manual entry is genuinely
   reachable at every point the camera is offered.

6. **Touch and legibility under the actual conditions.** Target sizes against gloved hands,
   contrast in low light and in direct sun on a crush pad, font sizes, the iOS input zoom
   behaviour on focus, safe-area insets, and whether any interaction requires precision a cold
   wet hand does not have. `viewport-fit=cover` is set; check that the CSS actually uses the
   safe-area variables.

7. **iOS Safari specifics.** Anything in the client that behaves differently or not at all on
   iPadOS Safari: storage eviction, autoplay and permission gestures, back-forward cache,
   PWA limitations, file and camera input handling.

8. **Dev and LAN access.** `vite.config.ts` sets `server.host` and no `allowedHosts`, on a
   vite version that enforces the Host header check. State exactly which access paths work
   (localhost, LAN by IP, a tunnel hostname, a local DNS name) and which are refused, and what
   the one-line fix is for the refused ones.

9. **Loading and feedback.** Every action that takes longer than a moment: is there a pending
   state, is the control disabled while in flight, and is there a timeout. A button with no
   feedback on a slow connection gets pressed three times.
...
```
**G-3-n. One line naming the issue.**

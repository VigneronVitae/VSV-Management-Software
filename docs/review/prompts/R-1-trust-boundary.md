...
The five files in `supabase/migrations/`. Without `0002` and `0003` this prompt cannot run at all; without `0001` and `0005` it runs at half scope. `tests/schema_assertions.sql`, `packages/cellar/src/walk.ts`, and `packages/core/src/kernel.ts` sharpen the findings and are not required.
...
Probe every write path named below under each role.
...
A producer cannot grant itself standing. Trust fields are written by the verifier, never by
whoever supplied the data. Stated as T0-4 in the spec and as hard rule two in `CLAUDE.md`,
and adopted from a tool-boundary pattern rather than a discipline, on the reasoning that a
boundary a model cannot talk its way past is worth more than a rule it is asked to follow.

## The question

Is it a boundary, or is it a rule?

Read every write path end to end: `packages/core/src/kernel.ts`, the walk screens in
`packages/cellar/src/`, all five migrations in `supabase/migrations/`, and
`tests/schema_assertions.sql`. Then answer:

1. Enumerate every path by which a value reaches `provenance` on `node` or `event`. For each,
   state who chooses the value and what refuses a wrong choice. A check constraint, a
   trigger, an RLS policy, a `security definer` function that strips and recomputes, and a
   comment asking politely are five different answers and only four of them are boundaries.

2. Enumerate every `security definer` function across the migrations. Each runs with the
   definer's rights and therefore bypasses RLS on everything it touches. For each: what does
   it write, what does it read, what does it validate about its arguments, and can a caller
   reach a row through it that RLS would refuse directly? `search_path` pinning counts here.

3. Trace the admission boundary for writes. Which authenticated principals may insert into
   `event`, `node`, `lineage`, `placement`, and `term`, and what does the database require of
   them beyond holding a token? Specifically: is there any table where the check is a client
   routing decision rather than a policy predicate? State where the write-admission boundary
   actually lives, by locator, rather than where the documents say it lives.

4. Consider an authenticated principal who signed up and never called `claim_account`, so
   holds a token and no `app_user` row. Enumerate everything that principal can read and
   write. Then state what the board, the task views, and any join against `app_user` do with
   rows that principal created.

5. Consider a cellar user acting in bad faith rather than in error. What is the most damaging
   thing they can do to the record that the schema permits, and how visible is it afterward?

Do not assume a policy holds because `tests/schema_assertions.sql` asserts it. The assertions
set the `sub` claim directly rather than receiving a JWT; that gap is named as S-7 and is in
scope. Where an assertion proves a policy and assumes the plumbing, say which half is which.
...
```
**R-1-n. One line naming the defect.**

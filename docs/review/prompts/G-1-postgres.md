...
The five files in `supabase/migrations/`. Nothing substitutes for these. `tests/schema_assertions.sql` and `supabase/config.toml` extend the review to what is already checked and how the instance is configured.
...
Apply, assert, and probe the constraints and functions.
...
All five files in `supabase/migrations/`, plus `tests/schema_assertions.sql` and
`supabase/config.toml`.

## Review this

1. **Data types and precision.** Every numeric column: is the precision right for what it
   holds, and does arithmetic across columns of different scales round the way the domain
   needs? Volumes, weights, fractions, temperatures, percentages. Check every place two
   numerics of different scale are multiplied or summed, and say where rounding accumulates.
   Check timestamp columns for `timestamptz` against `timestamp` and say what happens across a
   daylight-saving boundary during harvest.

2. **Constraints.** Every check, unique, exclusion, and foreign key. Are the constraints the
   ones the domain actually needs, and is anything enforced in a trigger that a constraint
   could enforce declaratively? Partial unique indexes: confirm each predicate is what the
   author meant. Flag any constraint that can be satisfied by an obviously wrong value.

3. **Indexes against query shapes.** Read the queries and functions, then check the index set
   against them. Report indexes with no query that uses them, and queries with no index that
   supports them. At the stated scale most sequential scans are fine, so only flag a missing
   index where you can name the query and say why it matters.

4. **The recursive traversals.** `block_composition`, `variety_composition`, and any other
   recursive CTE. Check for: termination on a cycle in `lineage`, correctness of the fraction
   arithmetic across multiple levels, behaviour when a fraction sums to less than one across
   siblings, and whether the recursion is bounded. State the depth and width at which each
   becomes slow, in concrete numbers.

5. **Function definitions.** For every function: volatility marker (`immutable`, `stable`,
   `volatile`) against what it actually does, `security definer` against `security invoker`
   and whether it needs the former, `search_path` pinning on every definer function,
   parameter validation, and behaviour on null arguments. A `volatile` function that is
   actually `stable` costs the planner real work in a join.

6. **Views.** `security_invoker` status on each, and what each one does under RLS. Confirm
   that a view does not widen access beyond the tables under it.

7. **Migration safety.** Each migration in order: does it apply cleanly to a database with
   data in it, not just to an empty one? Flag any statement that takes a long lock, rewrites a
   table, or drops something without a data path. State whether the sequence is replayable and
   what a rollback would look like, given there are no down migrations.

8. **Realtime publication.** `supabase_realtime` includes some tables. State exactly what is
   broadcast, to whom, and whether RLS applies to the replication stream as configured.

9. **Enums against lookup tables.** Migration 0004 moved several enums to a `term` table.
   Check the ones that remain and say whether each is correctly fixed rather than
   configurable, and what adding a value to each would cost.
...
```
**G-1-n. One line naming the issue.**

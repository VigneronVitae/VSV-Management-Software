...
A complete file listing of the tree, plus `docs/status-ledger.md`, `docs/sorry-ledger.md`, `docs/compost-ledger.md`, `CLAUDE.md`, and the root `package.json`. The listing is the hard requirement: this prompt is an audit of claims against what exists, so not knowing what exists guts it. Both files in `docs/session-reports/` and the typed headers of every document extend it.
...
Apply the migrations and run the assertion file; the rest of this prompt is a document audit and needs no database.
...
An artifact fills a function when it exercises the function. READMEs, specifications, and
roadmaps are evidence about intent and never evidence about filling. Where the two disagree,
the artifact governs. Emptiness is read, never declared.

## The question

Audit every maturity claim in this repository against the tree.

1. Extract every claim in `docs/status-ledger.md` graded Built and verified or In progress.
   For each, locate the artifact and the check that supports the grade. Report grade,
   locator, check, and whether the check exists as a committed runnable thing or only as a
   sentence in a session report.

2. `CLAUDE.md` defines done as a block of commands. For each: does the script exist in
   `package.json`, does it do what its name says, and can it pass today? Report any command in
   that block that cannot succeed against the tree as committed.

3. Both session reports in `docs/session-reports/` list things verified "by script." Locate
   each such script in the tree. Report every verification claimed by a script that is not
   committed, because an uncommitted check is a claim about a session rather than a property
   of the repository.

4. Check the typed headers. Every document carries `Depends on` and `Depended on by`.
   Reconstruct the full graph across all headered files including migrations, and report every
   one-directional edge, every edge pointing at a path that does not exist, and every headered
   file no other file points at.

5. Check the ledgers against each other and against themselves. Every sorry id referenced
   anywhere in the tree should exist in `docs/sorry-ledger.md`; every compost id referenced
   should exist in `docs/compost-ledger.md`; every status-ledger row graded Deferred should
   name a compost entry, since the ledger's own definition of Deferred requires one. Report
   gaps in both directions, and report any internal contradiction inside a single document.

6. Check style compliance mechanically: no em dashes in any tracked file or any commit
   message, since the convention is stated absolutely.

Output as a table of Claim / Locator / Supporting check / Verdict. A claim that is true and
unenforced is DIVERGENT rather than NOT A DEFECT, because the point of this prompt is which
claims a future edit can silently falsify.
...
```
**R-3-n. One line naming the defect.**

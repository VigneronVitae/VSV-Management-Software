...
is 116 migrations, `0001` through `0032`. This paragraph used to name a head sha and a
...
| B11 | `current_volume_l` render | `0` is falsy, so an emptied vessel displays as "unrecorded". The write side is correct. Reproduced in W-8 against a lot stored at `0.00`. **Closed by W-9 phase 5** at all three sites, two summary rows and the fill fraction, each now testing `=== null` rather than truthiness | 3 | closed |
...
statement of what this document is about: 116 migrations rather than five, the current head

...
// shared code moves down into core or gets duplicated, whichever is smaller, and
// two copies of a text input is the larger of those: the second copy is the one
// that stops matching.
//
// What stayed behind in cellar is `screen()`, which carries the practice band
// and the note button. That is cellar's chrome and the shop's chrome is not the
// same, which is the whole argument for a second periphery.
//
// Phone first. Single column, large targets, one action per view. There is no
// component model here on purpose.
